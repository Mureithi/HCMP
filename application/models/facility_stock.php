...
class Facility_Stock extends Doctrine_Record {
	...
	public static function getAllStock($facility) {
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT DISTINCT d.id,d.drug_name,d.unit_size,c.source_name from facility_stock fs, drug d, commodity_source c
...
");
	...
	public static function getAll($desc, $facility_c) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("
...
ORDER BY f.expiry_date ASC ");
	...
	public static function count_all_facility_stock() {
		$query = Doctrine_Query::create() -> select("kemsa_code,SUM( quantity ) AS quantity1") -> from(" facility_stock") -> where("status='1'") -> groupby("kemsa_code");
		$stocktake = $query -> execute();
		return $stocktake;
	}
	...
	public static function count_all_county_stock($county) {
		...
		$query = Doctrine_Query::create() -> select("facility_stock.kemsa_code, SUM( facility_stock.quantity ) as quantity1") -> from("facility_stock , districts, counties, facilities ") -> where("counties.id = districts.county") -> andWhere("districts.id = facilities.district") -> andWhere("counties.id =$county") -> andWhere("facility_stock.facility_code = facilities.facility_code") -> andWhere("facility_stock.STATUS ='1'") -> groupby("facility_stock.kemsa_code");
		$stocktake = $query -> execute();
		return $stocktake;
	}
	...
	public static function expiries($facility_c) {
		$query = Doctrine_Query::create() -> select("*") -> from("Facility_Stock") -> where("facility_code='$facility_c' and balance >0 and expiry_date BETWEEN CURDATE()AND DATE_ADD(CURDATE(), INTERVAL 6 MONTH)") -> andwhere("status=1");
...
SUM( f_s.balance ) / d.total_units ) * d.unit_cost, 1) AS total") -> from("Facility_Stock f_s, drug d") -> where("facility_code='$facility' and `expiry_date` <= NOW( ) and f_s.kemsa_code=d.id and STATUS =1 and balance >0") -> groupby("f_s.kemsa_code");
	...
	public static function get_facility_expired_stuff($date, $facility) {
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT d.id as drug_id,d.kemsa_code,d.unit_size,d.drug_name,d.unit_cost,f_s.batch_no,f_s.manufacture,
...
and f_s.STATUS ='1' GROUP BY d.id");
	...
	public static function get_exp_count($facility) {
		$query = Doctrine_query::create() -> select("batch_no") -> from("Facility_Stock") -> where("facility_code='$facility' and expiry_date<=NOW() and balance >0") -> andWhere("status='1'") -> groupBy("batch_no");
		$expire = $query -> execute();
	...
	public static function get_potential_count($facility) {
		$query = Doctrine_query::create() -> select("batch_no") -> from("Facility_Stock") -> where("facility_code='$facility' and expiry_date BETWEEN CURDATE()AND DATE_ADD(CURDATE(), INTERVAL 6 MONTH) and balance >0") -> andWhere("status='1'") -> groupBy("batch_no");
		$expire = $query -> execute();
	...
	public static function poo($drug_id) {
		$query = Doctrine_Query::create() -> select("SUM(`balance`) as total,expiry_date") -> from("Facility_Stock") -> where("kemsa_code='$drug_id'") -> andwhere("expiry_date BETWEEN CURDATE()AND DATE_ADD(CURDATE(), INTERVAL 12 MONTH)") -> groupby("MONTH(expiry_date) ASC");
	...
	public static function get_district_stock_level($distict) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT d.drug_name, ceil(SUM( fs.balance / d.total_units )) AS total
...
ORDER BY d.drug_name ASC  ");
	...
	public static function get_facility_stock_level($distict) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT d.drug_name, CEIL( SUM( fs.balance / d.total_units ) ) AS total, ifnull(temp.consumption_level,0) as consumption_level
...
ORDER BY d.drug_name ASC");
	...
	public static function get_district_drug_stock_level($distict, $drug_id) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT f.facility_name AS drug_name, CEIL( SUM( fs.balance / d.total_units ) ) AS total
FROM facility_stock fs, drug d, facilities f
WHERE fs.facility_code = f.facility_code
AND f.district =  '$distict'
...
ORDER BY d.drug_name ASC ");
	...
	public static function get_county_drug_stock_level_new($county_id, $category_id = NULL, $commodity_id = NULL, $district_id = null, $option = null, $facility_code = null) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT $computation
...
");
	...
	public static function get_county_drug_stock_level($county_id, $category_id = NULL, $commodity_id = NULL, $district_id = null, $option = null, $facility_code = null) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT d.drug_name, $computation
FROM facility_stock fs, drug d, facilities f, districts di
WHERE fs.facility_code = f.facility_code
AND f.district = di.id
AND fs.STATUS ='1'
...
ORDER BY d.drug_name ASC ");
	...
	public static function get_county_drug_consumption_level2($facilities_filter, $county_id, $district_filter, $commodity_filter, $year_filter, $plot_value_filter) {
		...
		if ($district_filter == 0 && $facilities_filter == 0 ) {
			$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT MONTH( fs.date_issued ) as monthno, $computation 
FROM facility_issues fs, drug d, facilities f, districts di, counties c
WHERE fs.facility_code = f.facility_code
AND f.district = di.id
AND fs.availability =  '1'
...
GROUP BY MONTH( fs.date_issued )  ");
	...
	public static function get_district_cost_of_exipries($distict) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT date_format( fs.expiry_date, '%b' ) as cal_month , ceil(sum( (fs.balance/ d.total_units) * d.unit_cost )) AS total
...
GROUP BY month( expiry_date ) asc");
	...
	public static function get_county_stock_out_trend($county_id, $year = NULL) {
		$year = isset($year) ? $year : date("Y");
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT count(distinct f_t.facility_id) as total,date_format( `start_date`, '%b' ) as month, 
 ...
 GROUP BY month( `start_date`) asc");
	...
	public static function get_county_cost_of_exipries_new($county_id, $year = null, $month = null, $district_id = null, $option = null, $facility_code = null) {
		...
		if ($month == "null") {
			...
			$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("select temp.cal_month, sum(temp.total) as total from (
...
");
	...
	public static function get_county_cost_of_exipries($county_id, $year = null, $district_id = null, $commodity_id = null, $option = null) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT date_format( fs.expiry_date, '%b' ) as cal_month ,$computation
FROM facility_stock fs, facilities f, drug d, counties c, districts di
WHERE fs.facility_code = f.facility_code
AND `expiry_date` <= NOW( )
AND DATE_FORMAT( fs.expiry_date,'%Y') =$year
...
GROUP BY month(expiry_date) asc");
	...
	public static function get_decom_count($district) {
		$query = Doctrine_Query::create() -> select("DISTINCT(facility_stock.facility_code)") -> from("facility_stock,facilities,districts") -> where("facility_stock.facility_code=facilities.facility_code") -> andWhere("facilities.district=districts.id") -> andWhere("facility_stock.status=2") -> andWhere("districts.id=$district");
		$order = $query -> execute();
		$order = count($order);
		//echo $order; exit;
		return ($order);
	...
	public static function get_county_max_date($county_id) {
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT MAX(stock_date) as date_ FROM `facility_stock` f_s, districts d, facilities f WHERE f.facility_code=f_s.`facility_code` and f.district=d.id and d.county=$county_id");
		return $inserttransaction;
	}
	...
	public static function get_decommisioned_count($county_id = null, $district_id = null) {
		...
		$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection() -> fetchAll("SELECT ifnull(COUNT(f_s.id),0) AS total
